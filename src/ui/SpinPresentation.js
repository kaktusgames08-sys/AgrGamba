...
export function createSpinPlan(durationMs, rareType = 'normal', random = Math.random) {
  ...
  const tickCount = degenerate ? 4 : turbo ? 6 : 8;
  const firstDelay = degenerate ? 42 : turbo ? 52 : 62;
  const finalDelay = degenerate ? 92 : turbo ? 145 : ultra ? 245 : 225;
  ...
  let fakeStops;
  if (degenerate) {
    fakeStops = random() < 0.4 ? 1 : 0;
  } else if (turbo) {
    fakeStops = 1;
  } else {
    fakeStops = 1 + (random() < (ultra ? 0.7 : 0.45) ? 1 : 0);
  }

  return {
    duration,
    chaosMs: Math.round(duration * (degenerate ? 0.30 : turbo ? 0.36 : 0.42)),
    chaosIntervalMs: ultra ? 28 : degenerate ? 34 : 40,
    slowdownDelays,
    fakeStops,
    fakeStopHoldMs: Math.round(degenerate ? range(70, 95, random) : turbo ? range(105, 135, random) : range(155, 215, random)),
    nearMissHoldMs: Math.round(degenerate ? range(90, 115, random) : turbo ? range(135, 165, random) : range(200, 250, random)),
    silenceMs: Math.round(degenerate ? range(72, 96, random) : turbo ? range(112, 142, random) : range(170, 215, random)),
